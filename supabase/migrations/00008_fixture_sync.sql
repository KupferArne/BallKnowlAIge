-- Epic 10: fixture sync from free sources (openfootball / football-data.org)
...
-- NULLs are distinct in Postgres uniques → demo rows without external_id stay fine
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'matches_tournament_external_unique'
  ) then
    alter table public.matches
      add constraint matches_tournament_external_unique
      unique (tournament_id, external_id);
  end if;
end $$;

comment on column public.matches.external_id is
  'Stable id from sync source (openfootball / football-data.org).';
...
begin
  ...
  loop
    ...
    values (tourney.id, ext, home, away, kick, hg, ag, st)
  ...
  -- Drop leftover demo/manual rows without an external id once real fixtures land
  if upserted > 0 then
